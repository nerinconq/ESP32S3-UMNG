...
function switchTab(tabName) {
    ...
    if (!tabConf) {
        // Handle non-data tabs (python, gpioview, config)
        $('display-title').textContent = tabName.toUpperCase();
        const toolbar = $('sensor-toolbar');
        if (toolbar) toolbar.style.display = 'none';
        
        // Specific panel handling
        $('config-panel').style.display = (tabName === 'config') ? 'block' : 'none';
        $('python-panel').style.display = (tabName === 'python') ? 'block' : 'none';
        $('gpio-viewer-panel').style.display = (tabName === 'gpioview') ? 'block' : 'none';
        
        // Hide hero card on non-data tabs
        const hero = document.querySelector('.hero-card');
        if (hero) hero.style.display = (['movimiento', 'rotacion', 'fuerza'].includes(tabName)) ? 'block' : 'none';
        const info = document.querySelector('.info-row');
        if (info) info.style.display = (['movimiento', 'rotacion', 'fuerza'].includes(tabName)) ? 'flex' : 'none';
        
        return;
    }

    // Show hero and toolbar for data tabs
    document.querySelector('.hero-card').style.display = 'block';
    document.querySelector('.info-row').style.display = 'flex';
    $('sensor-toolbar').style.display = 'flex';
    $('config-panel').style.display = 'none';
    $('python-panel').style.display = 'none';
    $('gpio-viewer-panel').style.display = 'none';
    ...
    if (invertBtn) invertBtn.style.display = (tabName === 'rotacion') ? 'block' : 'none';
    if (resetEncBtn) resetEncBtn.style.display = (tabName === 'rotacion') ? 'block' : 'none';
    if (tareBtn) tareBtn.style.display = (tabName === 'fuerza') ? 'block' : 'none';

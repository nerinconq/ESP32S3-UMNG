...
graph TD
    ...
    %% Lógica de Negocio
    subgraph Logic ["Lógica Híbrida"]
        NORMAL[Modo Normal: Medición + WiFi]
        EXPORT[Modo Exportación: Copia LittleFS -> USB]
    end
...
| :--- | :--- |
| `src/main.cpp` | Lógica principal y manejo de estados del sistema. |
| `src/idf_component.yml` | Dependencia nativa `espressif/usb_host_msc`. |
| `data/www/` | Frontend de la aplicación (Dashboard + Lógica JS). |
| `Planes_AI/` | Documentación técnica y planes de implementación. |

## Estado de Implementación
- [x] **H1-H4**: Sensores y UI Base.
- [x] **H5**: Preparación Sincronización Firebase.
- [x] **H6**: Modo Híbrido USB Host MSC (Oficial Espressif).
- [x] **H7**: Persistencia de Planes de Trabajo (Planes_AI).
